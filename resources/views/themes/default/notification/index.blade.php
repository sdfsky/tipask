    ...
    <div class="row">
        <div class="col-xs-12 col-md-9 main">
            ...
            <div class="stream-list widget-notify border-top">
                ...
                <section class="streams-item @if($notification->is_read==0) not_read @endif">
                    ...
                    @elseif(in_array($notification->type,['remove_question','remove_article','remove_answer']))
                    ...
                    @elseif(in_array($notification->type,['buy_video']))
                        <a href="{{ route('live.course.show',['id'=>$notification->source_id]) }}" target="_blank">{{ $notification->subject }}</a>
                    ...
                    @if($notification->content)
                        <blockquote class="text-fmt">@if(in_array($notification->type,['remove_question','remove_article','remove_answer']))原因：@endif{{ str_limit(strip_tags($notification->content),450) }}</blockquote>
                    @endif
                </section>
                @endforeach
            </div>
