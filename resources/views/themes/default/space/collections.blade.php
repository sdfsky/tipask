...
@section('space_content')
    <div class="stream-following">
        <ul class="nav nav-tabs mt-20">
            ...
            <li @if($source_type==='courses') class="active" @endif ><a href="{{ route('auth.space.collections',['user_id'=>$userInfo->id,'source_type'=>'courses']) }}">收藏的课程</a></li>
        </ul>
                ...
                @elseif($source_type=='courses')
                    <section class="stream-list-item">
                        ...
                        <div class="summary">
                            ...
                            <h2 class="title">
                                <a href="{{ route('live.course.show',['id'=>$collection['info']->id]) }}">{{ $collection->subject }}</a>
                            </h2>
                        </div>
                    </section>
                @else
