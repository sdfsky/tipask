...
class SettingController extends AdminController
{
    ...
    public function payment($type='alipay', Request $request){
        if($request->isMethod('post')){
            $envParams = [];
            if($type == 'global'){//全局配置
                $envParams['PAYMENT_OPEN'] = $request->input('pay_open',0);
                $envParams['PAY_CHARGE_RATE'] = $request->input('website_charge_rate',0);
            }else if($type == 'alipay'){//支付宝参数设置
                $envParams['ALIPAY_OPEN'] = $request->input('alipay_open',0);
                $envParams['ALIPAY_APP_ID'] = $request->input('alipay_appid','');
                $envParams['ALIPAY_NOTIFY_URL'] = $request->input('alipay_notify_url','');
                $envParams['ALIPAY_RETURN_URL'] = $request->input('alipay_return_url','');

                if($request->hasFile('alipay_public_key')){
                    $savePath = storage_path('app/authFiles');
                    $file = $request->file('alipay_public_key');
                    $fileName = 'alipay_public_key.pem';
                    $target = $file->move($savePath,$fileName);
                    if($target){
                        $envParams['ALIPAY_PUBLIC_KEY'] =  $savePath."/".$fileName;;
                    }
                }
                if($request->hasFile('alipay_private_key')){
                    $savePath = storage_path('app/authFiles');
                    $file = $request->file('alipay_private_key');
                    $fileName = 'app_private_key.pem';
                    $target = $file->move($savePath,$fileName);
                    if($target){
                        $envParams['ALIPAY_PRIVATE_KEY'] =  $savePath."/".$fileName;;
                    }
                }

            }else if($type == 'wechat'){//微信参数配置
                $envParams['WECHAT_PAY_OPEN'] = $request->input('wechat_open',0);
                $envParams['WECHAT_PUB_APP_ID'] = $request->input('wechat_pub_app_id','');
                $envParams['WECHAT_MCH_ID'] = $request->input('wechat_mch_id','');
                $envParams['WECHAT_PAY_API_KEY'] = $request->input('wechat_api_key','');
                $envParams['WECHAT_PAY_NOTIFY_URL'] = $request->input('wechat_notify_url','');
                if($request->hasFile('wechat_apiclient_cert')){
                    $savePath = storage_path('app/authFiles');
                    $file = $request->file('wechat_apiclient_cert');
                    $fileName = 'wechat_apiclient_cert.pem';
                    $target = $file->move($savePath,$fileName);
                    if($target){
                        $envParams['WECHAT_PAY_CERT'] =  $savePath."/".$fileName;;
                    }
                }
                if($request->hasFile('wechat_apiclient_key')){
                    $savePath = storage_path('app/authFiles');
                    $file = $request->file('wechat_apiclient_key');
                    $fileName = 'wechat_apiclient_key.pem';
                    $target = $file->move($savePath,$fileName);
                    if($target){
                        $envParams['WECHAT_PAY_KEY'] =  $savePath."/".$fileName;;
                    }
                }
            }
            Setting()->setEnvParams($envParams);
            return $this->success(route('admin.setting.payment'),'配置保存成功');
        }
        return view('admin.setting.payment')->with(compact('type'));
    }
